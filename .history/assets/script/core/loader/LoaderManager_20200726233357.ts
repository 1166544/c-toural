...
export class LoaderManager {
	...
	private loadedExternalUrls: Map<string, boolean>;

	private constructor() {
		this.loadedResource = new Map();
		this.loadedExternalUrls = new Map();
	}
	...
	public setExternalSprite(sprite: cc.Sprite, url: string, reActive = false) {
	...
	public setExternalSpriteFrame(sprite: cc.Sprite, frame: cc.SpriteFrame, url: string, reActive = false) {
		this.loadExternalAsset(
			url,
			gen_handler((tex: cc.Texture2D) => {
				if (!sprite || !cc.isValid(sprite.node)) {
	...
	public setSprite(sprite: cc.Sprite, url: string, reActive = false) {
		this.loadRes(
			url,
			gen_handler((tex: cc.Texture2D) => {
				if (!sprite || !cc.isValid(sprite.node)) {
	...
	private cacheExternalAsset(url: string) {
		this.loadedExternalUrls.set(url, true);
	}
	...
	public releaseAll(excludeMap = null) {
		...
		// 释放外部资源
		this.loadedExternalUrls.forEach((_, url) => {
