...
export class NetworkHttpService {
	private static _inst: NetworkHttpService;

	private constructor() {}

	public static getInst(): NetworkHttpService {
		if (!this._inst) {
			this._inst = new NetworkHttpService();
		}
		return this._inst;
	}

	public doGet(url: string, headers, params, cb: handler) {
		if (params) {
			if (url.indexOf('?') === -1) {
				url += '?';
			}
	...
	public doPost(url: string, headers, params, cb: handler) {
