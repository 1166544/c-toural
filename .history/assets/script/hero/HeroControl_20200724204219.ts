...
export class HeroControl extends cc.Component {
	...
	private jumpButtonClick(): void {
		...
		if (cc.director.getActionManager().getNumberOfRunningActionsInTarget(this.hero) === 0) {
			...
			//  跳跃上升
			let jumpUp: any = cc.jumpTo(1, HeroControl.heroX, this.hero.y, this.speedY, 1);

			//  添加一个回调函数，用于在动作结束时调用我们定义的其他方法
			let callback: any = cc.callFunc(this.jumpOver, this);

			// 添加动作列表
			let seq: any = cc.sequence(jumpUp, callback);
